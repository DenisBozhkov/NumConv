...
* ```-o``` – specifies output file. If output file is specified, the application will not write the whole text on the console. If the file does not exist, it would be created.
...
* ```-d``` – specifies custom digit set for the numeral system. The base of the system is determined by the size of the set and is not restricted like in ```-b``` and ```-B``` options. Should not be specified if ```-b``` or ```-B``` are already specified.
...
### Errors and warning
The application would not convert the text if:
